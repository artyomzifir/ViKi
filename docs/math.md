...
Текущий профиль по умолчанию — `stable-fused-hand-v3`. Он принудительно выбирает MediaPipe, даже если в `user_configuration.json` сейчас записан RTMPose.
...
Но текущий V3 использует `joints3d.npz`, а не это среднее.

---

## 12. Заполнение пропусков

Для каждого landmark и каждой координаты \(x,y,z\) независимо строится natural cubic spline:
...
Если валидных точек меньше четырёх:
...
Важно: функция называется `fill_se3_spline`, но математически это не SE(3)-сплайн. Это независимые координатные сплайны для каждой точки скелета. Геометрия костей на этом этапе не сохраняется.
