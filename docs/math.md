...
> **Расхождение с поставляемым дефолтом (проверено 2026-09-10).**
> Этот документ описывает профиль `stable-fused-hand-v3`. Дефолт продукта —
> `DEFAULT_PERCEPTION_PROFILE = stable-fused-hand-v2`, и он отличается в двух
> местах, оба помечены ниже: покадровая уверенность `omega` (§16) и
> калибровка уверенности. V3 не запускался ни на одной сцене до 2026-09-10 и
> не имеет защищённого артефакта. Замер E13 показал, что `hand_fit` у V2, V2.1
> и V3 **побайтово совпадает**: `omega` не является входом артикулированной
> подгонки и доходит только до дата-терма IK.
...
где \(e_{in}\) — `triangulation.reproj_inlier_px` профиля, он же порог отбора
инлайеров и `f_scale` робастной потери при уточнении (§8). В V1–V3
\(e_{in}=4\) px; кандидаты `fused-hand-reproj8-v1` и `-reproj16-v1` ставят 8 и
16 px.

То есть quality падает, если:

- часть камер не согласилась;
- reprojection error приближается к \(2e_{in}\);
- лучи камер имеют слабый baseline.

> **Важное следствие связанности (E7).** Одно и то же число задаёт и порог
> приёма, и знаменатель спада. При \(e_{in}=4\) спад рассчитан до 8 px, но
> гейт режет на 4 px, поэтому **верхняя половина спроектированного диапазона
> недостижима по построению**. Расширение \(e_{in}\) не добавляет новый
> механизм мягкого веса — оно включает уже написанный.
>
> Отсюда же: `landmark_confidence` **нельзя сравнивать между профилями с
> разным** \(e_{in}\). Один и тот же остаток 3 px даёт 0.63 при пороге 4 px и
> 0.91 при 16 px. Это смена линейки, а не рост качества.
...
> **Дефолт V2 этого не делает (E8).** Гейт полноты ладони живёт в поле профиля
> `palm_evidence`: `"all_observed"` у V3 и нового `stable-fused-hand-v2.1`,
> `"mean"` у V1/V2 и обоих reproj-кандидатов. При `"mean"` формула выше
> остаётся простым средним четырёх, а среднее четырёх чисел не равно нулю,
> когда нулю равны не все. Ладонь, наблюдённая на четверть, получает примерно
> четверть веса.
>
> Измерено на пяти защищённых бейзлайнах V2: **45–61% кадров, управляющих IK,
> имеют неполно наблюдённую ладонь**; 131–271 кадр на сцену ведётся одним-двумя
> реальными лендмарками из четырёх, с \(\omega\approx0.20\) при 1/4 и
> \(\approx0.38\) при 2/4 против \(\approx0.80\) при полной ладони.
>
> Прямой замер (E14) показал, что включение гейта в одиночку **ухудшает**
> слежение IK: оно снимает 45–61% взвешенных кадров, и солвер теряет опору.
> Убирать фабрикацию правильно, но не ценой опоры, которую она давала.
...
> **Это верно только для целиком невалидных кадров.** Частично наблюдённая
> ладонь при `palm_evidence="mean"` (дефолт V2) даёт \(\omega_t>0\) и
> притягивает робота к позе, ориентация которой собрана в том числе из
> интерполированных точек — см. врезку в §16.

> **Откуда берётся `hand_fit` (проверено 2026-09-10).** Артикулированный
> оверлей ставится в **активный** `cln.npz` эпизода. Защищённые бейзлайны под
> `intermediates/baselines/<профиль>/` его **не содержат**: `protect_baseline`
> пишет только при отсутствии файла (`if not target.exists()`), а
> `prepare_episode` вызывает её *до* `install_articulated_overlay`; повторный
> вызов с комментарием «Refresh the recorded comparison» ничего не делает.
> Поэтому любой прогон ретаргета с бейзлайна молча уходит на landmark-позы,
> хотя прод ведёт артикулированную подгонку. Решение о том, как это чинить, не
> принято.
...
> **Физическая длина подтверждена оператором 2026-09-10: 11 мм.** Это же
> значение стоит во всех дефолтах — `config.py`, оба конфигурационных JSON,
> дефолт роута `/api/pipeline` и форма вкладки Retarget. Четыре плана от
> 2026-09-08, лежащие в датасете, записаны с 50 мм: значение было введено в UI
> вручную на одну сессию и сохранилось только внутри самих планов. Их метрики
> (в частности `cup_grab` 33.8 мм / 33.2°) получены с TCP, смещённым на 39 мм
> вдоль оси инструмента, **и эталоном служить не могут**.
...
> **Допуск проверки выполнимости (изменён 2026-09-10).** Ограничение выше —
> мягкое, внутри QP. После решения траектория дополнительно проверяется
> жёстко, и до 2026-09-10 порог был \(-10^{-7}\) м, то есть 0.1 микрона.
> Это на четыре порядка жёстче собственного критерия сходимости солвера
> (\(10^{-3}\) рад \(\approx0.7\) мм движения TCP при радиусе 700 мм) и на
> пять порядков жёстче калибровки доски (\(\approx14\) мм), которая и задаёт,
> где находится ноль. На сцене `block-push`, где рука толкает брусок по столу,
> пол — активное ограничение: у профиля V2 запас составил \(+5\cdot10^{-7}\)
> мм (полмикрона над плоскостью), а у кандидата 8 px — \(-3.6\cdot10^{-2}\)
> мм, и эпизод из 897 кадров был отвергнут. Решала численная шелуха
> последнего разряда.
>
> Сейчас `FLOOR_TOLERANCE_M = 5\cdot10^{-4}` м (0.5 мм) — по-прежнему глубоко
> внутри ошибки калибровки. Допуски **раздельные по величинам**:
> `trajectory_constraint_margins` возвращает в одном словаре сустав (рад),
> скорость (рад/с), пол (м) и столкновение (м), и единый скалярный порог молча
> смешивал единицы. Self-collision оставлен строгим
> (`COLLISION_TOLERANCE_M = 10^{-7}`): это ограничение безопасности, и его
> запас не ограничен точностью калибровки доски.
