...
D435i observing a tabletop workspace, with a UR10 manipulator mounted at a
fixed position relative to that workspace. Reach is the binding constraint on
this choice rather than payload: with the base offset used here, target poses
lie between $525$ and $911$~mm from the base, so a UR3, whose reach is
$500$~mm, cannot cover the demonstrated workspace at all---a substitution
verified to leave between $12$ and $61$~per cent of every recording
unreachable, with tracking error on those frames two to three times that on
reachable ones. The Azure Kinect was selected for the
...
interpolated points. Measured across five recordings, between 45 and 61 per
cent of the frames carrying non-zero weight had an incompletely observed palm.
A stricter variant zeroes the frame unless every landmark in $\mathcal{I}$ was
triangulated,
\begin{equation}
  \omega^{\text{strict}}_t \;=\;
  \omega_t \cdot
  ...
  \label{eq:confstrict}
\end{equation}
Both are reported, because the stricter form is not uniformly better: it
removes a fabrication, but it also removes between 45 and 61 per cent of the
weighted frames, and the measurements in Chapter~\ref{ch:disc} show the
solver drifting further from the target for want of the support the fabricated
frames were supplying. Recovering that support---rather than reinstating the
fabrication---is identified as future work in
Section~\ref{sec:limitations}. The construction
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \emph{The gripper body is not the hand.} The target is the midpoint
        ...
        kinematically unreachable even when the arm's reach is ample: in one
        recording the target sits a median of $36$~mm above the plane while the
        achieved tool point is held $87$--$145$~mm above it, and the resulting
        vertical gap accounts for roughly $70$~per cent of the squared tracking
        error. Improving perception makes this discrepancy more visible, not
        less, because interpolation had been smoothing the lowest excursions
        upward.
