...
fusion is intentionally NOT performed here (see ``viki.perception.pipeline``). The
...
class SkeletonRecorder:
    ...
    def stop(self) -> str | None:
        ...
        all_ids = list(range(HAND_LM_COUNT))
        ...
        points = np.array(points_list, dtype=np.float32) if points_list else np.empty((0, HAND_LM_COUNT, 3), dtype=np.float32)
