...
export async function initializeFrontendConfig(config) {
  FRONTEND_CONFIG = {
    ...
    retarget: {
      ...
      wOrientation: config.RETARGET_W_ORIENTATION ?? 0.01,
      wVelocity: config.RETARGET_W_VELOCITY ?? 0.002,
      wAcceleration: config.RETARGET_W_ACCELERATION ?? 0.00002,
      wPosture: config.RETARGET_W_POSTURE ?? 0.0001,
      huberDelta: config.RETARGET_HUBER_DELTA ?? 0.05,
      confidenceFloor: config.RETARGET_CONFIDENCE_FLOOR ?? 0.05,
      lmDamping: config.RETARGET_LM_DAMPING ?? 0.00001,
      maxIterations: config.RETARGET_MAX_ITERATIONS ?? 12,
      maxStepRad: config.RETARGET_MAX_STEP_RAD ?? 0.2,
      convergenceRad: config.RETARGET_CONVERGENCE_RAD ?? 0.0002,
      qpSolver: config.RETARGET_QP_SOLVER ?? 'osqp',
      collisionEnabled: config.RETARGET_COLLISION_ENABLED ?? true,
